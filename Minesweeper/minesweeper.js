    ...
    if (btn.context === "💣") {
      ...
      for (let i = 0; i < closedbuttons.length; i++) {
        ...
        if (btnCoords[0] === checkBtnCoords[0] - 1) {
          if (
            checkBtnCoords[1] === btnCoords[1] ||
            checkBtnCoords[1] === btnCoords[1] + 1 ||
            checkBtnCoords[1] === btnCoords[1] - 1
          ) {
...
function makeBoard(rows, cols, mines, parent) {
  ...
  // check if mines < buttons
  if (mines > rows * cols) {
    console.error("more mines than buttons");
    return;
  }
  // more rows or cols than intended
  if (rows > 10 || cols > 10) {
    console.error("more rows or cols than intended");
    return;
  }
  ...
  if (mines <= 0) {
    console.error("less than 1 mine");
    return;
  }
  ...
  for (let i = 0; i < btnsList.length; i++) {
    ...
    for (let a = 0; a < minesList.length; a++) {
      ...
      if (btnCoords[0] === mineCoords[0] - 1) {
        if (
          mineCoords[1] === btnCoords[1] ||
          mineCoords[1] === btnCoords[1] + 1 ||
          mineCoords[1] === btnCoords[1] - 1
        ) {
